import { AssetList } from '@chain-registry/types';
const info: AssetList = {
  $schema: '../../assetlist.schema.json',
  chainName: 'nvnmchaintestnet',
  assets: [{
      description: 'Wrapped mantraUSD bridged from MANTRA Dukong testnet over IBC; the gas and staking token of NVNM Chain Testnet.',
      denomUnits: [{
          denom: 'ibc/00E24F149F01782E2C423BDB114C98D6C657DFC583124389192187BF8DC3A1AD',
          exponent: 0,
          aliases: ['erc20:0x36A28B8C8FFD00bEE440dB8273D57451FBeAf767']
        }, {
          denom: 'wmantrausd',
          exponent: 18
        }],
      typeAsset: 'ics20',
      base: 'ibc/00E24F149F01782E2C423BDB114C98D6C657DFC583124389192187BF8DC3A1AD',
      name: 'Wrapped mantraUSD',
      display: 'wmantrausd',
      symbol: 'wmantraUSD',
      traces: [{
          type: 'ibc',
          counterparty: {
            chainName: 'mantrachaintestnet2',
            baseDenom: 'erc20:0x36A28B8C8FFD00bEE440dB8273D57451FBeAf767',
            channelId: 'channel-13'
          },
          chain: {
            channelId: 'channel-1',
            path: 'transfer/channel-1/erc20:0x36A28B8C8FFD00bEE440dB8273D57451FBeAf767'
          }
        }],
      logoURIs: {
        png: 'https://raw.githubusercontent.com/cosmos/chain-registry/master/mantrachain/images/wmantraUSD-token.png',
        svg: 'https://raw.githubusercontent.com/cosmos/chain-registry/master/mantrachain/images/wmantraUSD-token.svg'
      },
      images: [{
          imageSync: {
            chainName: 'mantrachaintestnet2',
            baseDenom: 'erc20:0x36A28B8C8FFD00bEE440dB8273D57451FBeAf767'
          },
          png: 'https://raw.githubusercontent.com/cosmos/chain-registry/master/mantrachain/images/wmantraUSD-token.png',
          svg: 'https://raw.githubusercontent.com/cosmos/chain-registry/master/mantrachain/images/wmantraUSD-token.svg',
          theme: {
            circle: true
          }
        }]
    }]
};
export default info;